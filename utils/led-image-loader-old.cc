...
void showImage(FileInfo *file_img, RGBMatrix *matrix, FrameCanvas *offscreen_canvas) {
  file_img->params.wait_ms = 2;
  file_img->params.loops = 1;
...
int main(int argc, char *argv[]) {
  ...
  std::map<int, std::string> mapColors;
    ...
    mapColors[2]="orange";
    mapColors[3]="green";
  std::map<int, directions> mapDirections;
    mapDirections[0] = "top";
  ...
  std::vector<std::string> allColors = {"green", "blue", "red", "orange"};
  std::vector<std::string> allDirections = {"top", "bottom", "left", "right"};
  ...
  for(int c = 0; c<num_colors; c++) {
    ...
    for(int d = 0; d<num_directions; d++) {
      ...
      if (LoadImageAndScale(filename, matrix->width(), matrix->height(), fill_width, fill_height, &image_sequence, &err_msg)) {
        ...
        //ImageParams &params = file_imgs[index_images]->params;
        ...
        dictionaryFileImages[file_name_string] = file_img;
      }
    }
  }
    ...
    if (server < 0)
      ...
      while (server > 0) 
      {
          ...
          while (!isExit) {
              ...
              std::string key(prefix_file+name+suffix_file);
              cout << "KEY: " << key << endl;
